...
public class check {
    ...
    // Phương thức kiểm tra và hiển thị danh sách quá hạn trả trong ngày
    public static void checkAndDisplayOverdue(List<BorrowSlip> borrowSlips, List<User> users, List<Book> books) {
        try {
            ...
            for (BorrowSlip slip : borrowSlips) {
                Date returnDate = sdf.parse(slip.getReturnDate());
                ...
                if (returnDate.before(today)) {
                    ...
                    System.out.println("Ngày trả dự kiến: " + slip.getReturnDate());
                    System.out.println();
                }
            }
